...
app.get('/api/users/:_id/logs', async function (req, res) {
  try {
    ...
    const jsonResponse = {count: user.count, log: []};
    let logs = []

    for(let i = 0; i < user.log.length; i++) {
      const currentLog = {description: user.log[i].description, duration: user.log[i].duration, date: user.log[i].date};
